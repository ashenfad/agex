...
def test_gemini_client_message_conversion():
    ...
    with patch.object(client, "client") as mock_client:
        ...
        # First message should contain system message prepended
        assert gemini_messages[0]["role"] == "user"
        assert (
            "System: You are a helpful assistant."
            in gemini_messages[0]["parts"][0]["text"]
        )
        assert "User: Hello" in gemini_messages[0]["parts"][0]["text"]
...
def test_gemini_client_multiple_system_messages():
    ...
    with patch.object(client, "client") as mock_client:
        ...
        first_message_text = gemini_messages[0]["parts"][0]["text"]
        expected_system = (
            "You are a helpful assistant.\n\nYou are also very knowledgeable."
        )
        assert f"System: {expected_system}" in first_message_text
        assert "User: Hello" in first_message_text
